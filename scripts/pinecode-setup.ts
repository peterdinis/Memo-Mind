...
async function setupPinecone() {
  ...
  try {
    ...
    console.log('✅ Available indexes:', indexNames.length > 0 ? indexNames : 'None');
    ...
    while (!isReady && attempts < maxAttempts) {
      ...
      try {
        ...
        console.log(`⏰ Attempt ${attempts}/${maxAttempts}: Index status - ${status}`);
    ...
    if (!isReady) {
      console.log('❌ Index creation timeout. Please check Pinecone console.');
      process.exit(1);
    }
