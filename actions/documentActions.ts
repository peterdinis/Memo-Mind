...
async function storeDocumentChunksInPinecone(
    documentId: string,
    chunks: string[],
    documentName: string,
): Promise<void> {
    try {
        ...
        for (let i = 0; i < chunks.length; i++) {
            try {
                ...
                console.error(
                    `Error creating embedding for chunk ${i}:`,
                    error,
                );
        ...
        throw new Error(
            `Failed to store document chunks in vector database: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
    }
}
...
async function createEmbedding(text: string): Promise<number[]> {
    try {
        ...
        throw new Error(
            `Failed to create text embedding: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
    }
}
