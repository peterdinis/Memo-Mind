...
export async function chatWithDocument(
    ...
    try {
        ...
        if (document.status !== 'processed') {
            throw new Error(
                `Document is still ${document.status}. Please wait.`,
            );
        }
        ...
        if (!relevantChunks) {
            return {
                response:
                    "I couldn't find relevant information in the document to answer your question. Could you try rephrasing or asking about something else in the document?",
            };
        }
