...
const parseDocumentMetadata = (metadata: unknown): DocumentMetadata => {
  ...
  if (Array.isArray(rawMetadata.tags) && rawMetadata.tags.every(tag => typeof tag === 'string')) {
  ...
  if (typeof rawMetadata.status === 'string' && 
  ...
  .action(async ({ parsedInput: { query, folder, limit, offset } }) => {
    ...
    const filteredFiles = (files || [])
      .filter(file => 
        ...
        file.name.replace(/^\d+_/, '').toLowerCase().includes(query.toLowerCase())
      );
...
export const searchByMetadataAction = authenticatedAction
  ...
  .action(async ({ parsedInput: { metadata, folder, limit, offset } }) => {
    ...
    if (error) {
      ...
      throw new Error(`Failed to search files by metadata: ${error.message}`);
    }
    ...
    const filteredFiles = (files || [])
      .filter(file => {
        ...
        return Object.entries(metadata).every(([key, value]) => {
          ...
          if (typeof value === 'string' && typeof fileValue === 'string') {
            return fileValue.toLowerCase().includes(value.toLowerCase());
          }
          ...
          if (Array.isArray(value) && Array.isArray(fileValue)) {
            return value.every(v => 
              fileValue.some(fv => 
                typeof fv === 'string' && fv.toLowerCase().includes(v.toLowerCase())
              )
            );
          }
...
export const advancedSearchAction = authenticatedAction
  ...
  .action(async ({ parsedInput: filters }) => {
    ...
    const filteredFiles = (files || []).filter(file => {
      ...
      const fileDisplayName = file.name.replace(/^\d+_/, '').toLowerCase();
      const fileDate = new Date(file.created_at);
      const fileSize = fileMetadata.size || 0;

      // Filter by search query
      if (filters.query && !fileName.includes(filters.query.toLowerCase()) && 
      ...
      // Filter by category
      if (filters.category && fileMetadata.category !== filters.category) {
        return false;
      }
