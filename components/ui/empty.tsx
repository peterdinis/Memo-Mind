...
function Empty({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="empty"
      className={cn(
        "flex min-w-0 flex-1 flex-col items-center justify-center gap-8 rounded-xl border border-dashed p-12 text-center text-balance bg-muted/20 backdrop-blur-sm",
        className
      )}
      {...props}
    />
  ...
  "flex shrink-0 items-center justify-center mb-4 [&_svg]:pointer-events-none [&_svg]:shrink-0 transition-all duration-300",
  {
    variants: {
      variant: {
        ...
        success: "bg-gradient-to-br from-green-100 to-green-50 text-green-600 dark:from-green-900/20 dark:to-green-900/10 flex size-20 shrink-0 items-center justify-center rounded-2xl border border-green-200 dark:border-green-800 [&_svg:not([class*='size-'])]:size-10",
        warning: "bg-gradient-to-br from-amber-100 to-amber-50 text-amber-600 dark:from-amber-900/20 dark:to-amber-900/10 flex size-20 shrink-0 items-center justify-center rounded-2xl border border-amber-200 dark:border-amber-800 [&_svg:not([class*='size-'])]:size-10",
...
function EmptyDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <div
      ...
      className={cn(
        "text-muted-foreground text-lg/relaxed [&>a:hover]:text-primary [&>a]:font-medium [&>a]:underline [&>a]:underline-offset-4",
        className
      )}
      {...props}
    />
...
function EmptyActions({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-wrap items-center justify-center gap-4 mt-2",
        className
      )}
      {...props}
    />
...
function EmptyFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "mt-6 text-base text-muted-foreground/80",
        className
      )}
      {...props}
    />
